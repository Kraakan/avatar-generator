...
async def generate():
    ...
    if form.validate_on_submit():
        ...
        if extension == "svg": #Convert svg to png
            svg = open("flask_app/static/" + initial_image, "r").read()
            png_image_name = "input/" + initial_image_name +'.png'
        ...
        if model_selection != -1:
            ...
            if task[1] == "launching":
                process_checking_thread = threading.Thread(name="process checker", target=check_process, kwargs = {"delay": 3})
...
async def tune():
    ...
    if form.validate_on_submit():
        ...
        # TODO: Don't start process checker unless task is actually launched.
        if task[1] == "launching":
            process_checking_thread = threading.Thread(name="process checker", target=check_process, kwargs = {"delay": 10})
            process_checking_thread.start()
        return flask.render_template('tasks.html', queue =  queue.q, highlight = task, status = app_status)
